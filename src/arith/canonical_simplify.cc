...
bool CastIsSafe(PrimType dtype, PrimExpr value, AnalyzerObj* analyzer) {
  ...
  int64_t ubound = Downcast<IntImm>(max_value(runtime_dtype))->value;
  int64_t lbound = Downcast<IntImm>(min_value(runtime_dtype))->value;
...
PrimExpr CanonicalSimplifier::Impl::VisitExpr_(const LTNode* op) {
  ...
  if (const auto* lhs = expr.as<SumExprNode>()) {
    ...
    TVM_FFI_ICHECK(DataType(extra->ty().dtype()) == DataType(dtype.dtype()));
