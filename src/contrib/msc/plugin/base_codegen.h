...
class BasePluginCodeGen {
 public:
  ...
  virtual const Map<ffi::String, ffi::String> GetBuildSources(const std::string& print_options = "") {
    Map<ffi::String, ffi::String> sources;
    // plugin sources
    for (const auto& name : ListPluginNames()) {
      const auto& plugin = GetPlugin(name);
      // attr declare
      const ffi::String& attr_macro = "TVM_CONTRIB_MSC_" + StringUtils::Upper(plugin->name) + "_ATTR_H_";
      ...
      // op decalre
      const ffi::String& op_macro = "TVM_CONTRIB_MSC_" + StringUtils::Upper(plugin->name) + "_OP_H_";
  ...
  virtual const Map<ffi::String, ffi::String> GetManagerSources(const std::string& print_options = "") {
    Map<ffi::String, ffi::String> sources;
 ...
 protected:
  ...
  void CodeGenSafeCall(const PluginExtern& extern_func,
                       const Array<ffi::String>& call_args = Array<ffi::String>(), const ffi::String& ret = "") {
  ...
  void CodeGenPreCmake(const std::set<ffi::String>& devices,
                       const Map<ffi::String, ffi::String>& extra_flags = Map<ffi::String, ffi::String>()) {
  ...
  void CodeGenPostCmake(const std::set<ffi::String>& devices,
                        const Array<ffi::String>& extra_includes = Array<ffi::String>(),
                        const Array<ffi::String>& extra_libs = Array<ffi::String>()) {
    const auto& p_name = this->config()->project_name;
    stack_.line()
  ...
  std::vector<std::unordered_map<int, ffi::String>> GetDtypeMatrix(const Plugin& plugin) {
    ...
    if (plugin->support_dtypes.size() == 0) {
      ...
      Array<ffi::String> templates;
      Array<Array<ffi::String>> condidates;
  ...
  const Map<ffi::String, ffi::String> GetTensorDtypes(const Plugin& plugin,
                                            const std::unordered_map<int, ffi::String>& dtypes) {
    Map<ffi::String, ffi::String> tensor_dtypes;
