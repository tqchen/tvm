...
void LayoutsFinder::VisitExpr_(const CallNode* call_node) {
  ...
  if (func.defined()) {
    const auto& layouts_opt = func->GetAttr<Map<ffi::String, ffi::String>>(msc_attr::kInputLayouts);
...
const MSCGraph GraphBuilder::Build(const Function& func) {
  ...
  Array<ffi::String> input_names, output_names;
...
const MSCJoint GraphBuilder::AddNode(const Expr& expr, const Optional<Expr>& binding_var,
  ...
  if (plugin.defined()) {
    ...
    if (target_funcs_.count(op)) {
      const auto& opattrs_opt = target_funcs_[op]->GetAttr<Array<ffi::String>>(msc_attr::kOpattrs);
  ...
  Map<ffi::String, MSCTensor> node_weights;
  if (plugin.defined()) {
    const auto& call = Downcast<Call>(expr);
    if (call->args.size() == 1) {
...
const MSCPrim GraphBuilder::MatchOrCreatePrim(const PrimExpr& prim, const ffi::String& optype,
                                              ...
                                              const Map<ffi::String, ffi::String>& attrs) {
  if (prim_map_.count(prim)) {
    return prim_map_[prim];
  }
  const auto& op_ =
...
const std::tuple<ffi::String, String, ffi::String> GraphBuilder::ParseFunc(const Function& func) {
...
TVM_FFI_STATIC_INIT_BLOCK({
  ...
  refl::GlobalDef()
      .def("msc.core.BuildFromRelax",
           [](const IRModule& module, const ffi::String& entry_name, const ffi::String& options) -> MSCGraph {
      ...
      .def("msc.core.GetRelaxWeights",
           [](const IRModule& module, const ffi::String& entry_name) -> Map<MSCTensor, Tensor> {
