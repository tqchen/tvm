...
class BaseCodeGen {
 public:
  ...
  /*! \brief Get sources*/
  virtual const Map<ffi::String, ffi::String> GetSources(const std::string& print_options = "") = 0;
 ...
 protected:
  ...
  const std::stack<Array<ffi::String>> scopes() const { return scopes_; }
 ...
 private:
  MSCGraph graph_;
  std::stack<Array<ffi::String>> scopes_;
};
