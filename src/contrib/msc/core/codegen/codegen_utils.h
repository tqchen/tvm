...
#define CODEGEN_MEMBERS                                                                            \
 public:                                                                                           \
  virtual const ffi::String DType(const DataType& dtype) { return runtime::DLDataTypeToString(dtype); } \
                                                                                                   \
 protected:                                                                                        \
  const std::shared_ptr<ConfigType> config() { return config_; }                                   \
  const Map<ffi::String, ffi::String> prims() { return prims_; }                                             \
  ...
  const ffi::String IdxWeightBase(const MSCJoint& node, const ffi::String& wtype, bool process = true) {     \
    return helper_.IdxWeightBase(node, wtype, "", process && config()->use_tools);                 \
  }                                                                                                \
  const Array<ffi::String> GetPrims(const MSCTensor& tensor) {                                          \
 ...
 private:                                                                                          \
  ...
  Map<ffi::String, ffi::String> prims_;                                                                      \
  HelperType helper_;
...
class CodeGenUtils {
 public:
   ...
   * \return The String.
   */
  TVM_DLL static const ffi::String IdxOutput(const MSCJoint& node, const ffi::String& prefix, int idx = 0,
                                        const ffi::String& suffix = "");
   ...
   * \return The String.
   */
  TVM_DLL static const ffi::String IdxInput(const MSCJoint& node, const ffi::String& prefix, int idx = 0,
                                       const ffi::String& suffix = "");
   ...
   * \return The prims.
   */
  TVM_DLL static const Array<ffi::String> GetPrims(const MSCTensor& tensor,
                                              const Map<ffi::String, ffi::String>& prims);
  /*!
...
class BaseCodeGenHelper {
 public:
  ...
  virtual const ffi::String IdxInputBase(const MSCJoint& node, const ffi::String& prefix = "", int idx = 0,
  ...
  virtual const ffi::String IdxOutputBase(const MSCJoint& node, const ffi::String& prefix = "", int idx = 0,
                                     const ffi::String& suffix = "", bool mark_exit = false) {
    if (mark_exit) {
  ...
  virtual const ffi::String Comment(const MSCJoint& node, const ffi::String& prefix = "",
                               const Map<ffi::String, ffi::String>& prims = Map<ffi::String, ffi::String>()) {
