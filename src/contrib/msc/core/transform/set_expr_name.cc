...
class FuncNameGetter : public ExprVisitor {
 public:
  explicit FuncNameGetter(const Array<ffi::String>& arg_names) : arg_names_(arg_names) {}

  /*! \brief Get the attributes from prim value as Map<ffi::String, ffi::String>*/
  ffi::String HintName(const Expr& expr) {
    name_ = "";
...
class RelaxExprNameSetter : public ExprVisitor {
 public:
  explicit RelaxExprNameSetter(const IRModule& ref_module, const ffi::String& target,
                               const Map<ffi::String, ffi::String>& var_names)
  ...
  void VisitBindingBlock(const BindingBlock& block) final {
    ...
    const ffi::String& unique_name = StringUtils::Join(block_stack_, ".");
  ...
  void VisitExpr_(const ConstantNode* val) {
    ...
    const ffi::String& unique_name = GetUniqueName(ffi::GetRef<Constant>(val), "const");
  ...
  void VisitBinding_(const VarBindingNode* binding, const ShapeExprNode* val) {
    ExprVisitor::VisitBinding_(binding, val);
    const ffi::String& unique_name = GetUniqueName(ffi::GetRef<ShapeExpr>(val), "shape");
    if (unique_name != SpanUtils::GetAttr(val->span, msc_attr::kName)) {
      val->span = SpanUtils::SetAttr(val->span, msc_attr::kName, unique_name);
    }
    expr_names_.Set(binding->var, unique_name);
  ...
  void VisitBinding_(const VarBindingNode* binding, const TupleNode* val) {
    ExprVisitor::VisitBinding_(binding, val);
    const ffi::String& unique_name = GetUniqueName(ffi::GetRef<Tuple>(val), "tuple");
    if (unique_name != SpanUtils::GetAttr(val->span, msc_attr::kName)) {
      val->span = SpanUtils::SetAttr(val->span, msc_attr::kName, unique_name);
    }
    expr_names_.Set(binding->var, unique_name);
  ...
  void VisitBinding_(const VarBindingNode* binding, const FunctionNode* val) {
    ExprVisitor::VisitBinding_(binding, val);
    const auto& name_opt = val->GetAttr<ffi::String>(attr::kComposite);
  ...
  void VisitBinding_(const VarBindingNode* binding, const CallNode* val) {
    ...
    if (var_names_.count(binding->var->name_hint())) {
      ...
      if (op_name == "relax.call_dps_packed" && val->args[0]->IsInstance<ExternFuncNode>()) {
        ...
        const ffi::String& input_name = GetUniqueName(val->args[1], "plugin_inputs");
    ...
    if (name_hint.size() > 0) {
      ...
      for (size_t i = 0; i < input_types.size(); i++) {
        ...
        if (const auto* c_node = val->args[i].as<ConstantNode>()) {
          const ffi::String& const_name = SpanUtils::GetAttr(c_node->span, msc_attr::kName);
 ...
 private:
  const ffi::String GetUniqueName(const Expr& expr, const ffi::String& name_hint) {
    ffi::String expr_name = SpanUtils::GetAttr(expr->span, msc_attr::kName);
    if (expr_name.size() == 0) {
      expr_name = name_hint;
    }
  ...
  const ffi::String GetFuncType(const Function& func) {
    ffi::String optype;
    const auto& comp_opt = func->GetAttr<ffi::String>(attr::kComposite);
    const auto& code_opt = func->GetAttr<ffi::String>(attr::kCodegen);
  ...
  const ffi::String GetFuncName(const Call& call, const Function& func) {
    ffi::String name;
    // get from unique
  ...
  Map<ffi::String, Expr> setted_names_;
  Map<ffi::String, ffi::String> constant_consumers_;
  std::set<ffi::String> setted_blocks_;
  Array<ffi::String> block_stack_;
  Map<Expr, ffi::String> expr_names_;
  Map<Expr, Function> local_funcs_;
  IRModule ref_module_;
  ffi::String target_;
  Map<ffi::String, ffi::String> var_names_;
};  // class ExprNameSetter

void SetRelaxExprName(const IRModule& ref_module, const Expr& e, const ffi::String& target,
                      const Map<ffi::String, ffi::String>& var_names) {
...
Pass SetRelaxExprName(const ffi::String& entry_name, const ffi::String& target,
                      const Map<ffi::String, ffi::String>& var_names) {
