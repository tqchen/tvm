...
const ffi::String TensorRTOpCode::DType(const DataType& dtype) {
  const ffi::String& dtype_name = BaseOpCode<TensorRTCodeGenConfig, TensorRTCodeGenHelper>::DType(dtype);
...
const ffi::String TensorRTOpCode::ToDims(const Array<Integer>& dims, bool use_ndim) {
...
void TensorRTOpCode::SetLayerByDimsAttr(const ffi::String& method, const ffi::String& key, bool use_ndim) {
...
void TensorRTOpCode::SetLayerByDimsValue(const ffi::String& method, const Array<Integer>& value,
                                         bool use_ndim) {
  stack_.func_call("set" + method, std::nullopt, DocUtils::ToPtr(IdxNode()))
      .call_arg(ToDims(value, use_ndim));
}
...
GetTensorRTOpCodes() {
  static auto map = std::make_shared<std::unordered_map<ffi::String, std::shared_ptr<TensorRTOpCode>>>();
