...
void TensorRTCodeGen::CodeGenClassDefine() {
  ...
  if (config()->use_tools) {
    ...
    before_build_codes_ =
        pf(GetStepCtx(), "before_build", graph()->name, config()->tools_tag).cast<Array<ffi::String>>();
  }
  ...
  Array<ffi::String> batch_flags{"MIN", "MAX", "OPT"};
  ...
  if (config()->use_tools) {
    const auto pf = tvm::ffi::Function::GetGlobalRequired("msc_tool.codegen_step");
    after_build_codes_ =
        pf(GetStepCtx(), "after_build", graph()->name, config()->tools_tag).cast<Array<ffi::String>>();
  }
...
const ffi::String TensorRTCodeGen::CppDType(const DataType& dtype) {
  const ffi::String& dtype_name = CppCodeGen<TensorRTCodeGenConfig, TensorRTCodeGenHelper>::DType(dtype);
  if (dtype_name == "int32") {
    return "int";
  }
  if (dtype_name == "int64") {
...
const ffi::String TensorRTCodeGen::ToDims(const Array<Integer>& dims, bool use_ndim) {
...
const Map<ffi::String, ffi::String> TensorRTCodeGen::GetTensorCtx(const MSCTensor& tensor) {
  Map<ffi::String, ffi::String> tensor_ctx;
...
const Map<ffi::String, ffi::String> TensorRTCodeGen::GetStepCtx() {
  Map<ffi::String, ffi::String> step_ctx;
  ...
  refl::GlobalDef()
      .def("msc.framework.tensorrt.GetTensorRTSources",
           [](const MSCGraph& graph, const ffi::String& codegen_config,
              const ffi::String& print_config) -> Map<ffi::String, ffi::String> {
...
Array<ffi::Module> MSCTensorRTCompiler(Array<Function> functions,
                                       Map<ffi::String, ffi::Any> target_option,
