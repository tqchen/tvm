...
class TensorRTCodeGen : public CppCodeGen<TensorRTCodeGenConfig, TensorRTCodeGenHelper> {
 ...
 protected:
  ...
  const Map<ffi::String, ffi::String> GetTensorCtx(const MSCTensor& tensor) final;

  /*! \brief Get the step context for codegen_step*/
  const Map<ffi::String, ffi::String> GetStepCtx() final;
  ...
  const ffi::String ToDims(const Array<Integer>& dims, bool use_ndim = true);

 private:
  Array<ffi::String> before_build_codes_;
  Array<ffi::String> after_build_codes_;
};
