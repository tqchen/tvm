...
class TensorRTOpCode : public BaseOpCode<TensorRTCodeGenConfig, TensorRTCodeGenHelper> {
 ...
 protected:
  ...
  template <typename T>
  void SetLayerByDimsValue(const ffi::String& method, const std::vector<T>& value, bool use_ndim = true);
  void SetLayerByDimsValue(const ffi::String& method, const Array<Integer>& value, bool use_ndim = true);
};
