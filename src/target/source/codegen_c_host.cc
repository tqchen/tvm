...
ffi::Module BuildCHost(IRModule mod, Target target) {
  ...
  if (mod->GetAttr<Map<GlobalVar, ffi::String>>("device_contexts") != nullptr) {
    Map<GlobalVar, ffi::String> device_contexts =
        mod->GetAttr<Map<GlobalVar, ffi::String>>("device_contexts").value();
