...
double GetArchVersion(Array<ffi::String> mattr) {
...
double GetArchVersion(Optional<Array<ffi::String>> attr) {
  if (!attr) {
    return false;
  }
...
bool IsAArch32(Optional<ffi::String> mtriple, Optional<ffi::String> mcpu) {
  if (mtriple) {
...
bool IsAArch64(Optional<ffi::String> mtriple) {
  if (mtriple) {
...
bool IsArch(TargetJSON attrs) {
  Optional<ffi::String> mtriple = Downcast<Optional<ffi::String>>(attrs.Get("mtriple").value_or(nullptr));
  Optional<ffi::String> mcpu = Downcast<Optional<ffi::String>>(attrs.Get("mcpu").value_or(nullptr));

  return IsAArch32(mtriple, mcpu) || IsAArch64(mtriple);
}

bool CheckContains(Array<ffi::String> array, ffi::String predicate) {
...
static TargetFeatures GetFeatures(TargetJSON target) {
  ...
  Optional<ffi::String> mtriple = Downcast<Optional<ffi::String>>(target.Get("mtriple").value_or(nullptr));
  Optional<ffi::String> mcpu = Downcast<Optional<ffi::String>>(target.Get("mcpu").value_or(nullptr));
  ...
  Array<ffi::String> targets = llvm_backend.GetAllLLVMTargets();
  ...
  Map<ffi::String, ffi::String> features = llvm_target.GetAllLLVMCpuFeatures();
...
static Array<ffi::String> MergeKeys(Optional<Array<ffi::String>> existing_keys) {
  const Array<ffi::String> kExtraKeys = {"arm_cpu", "cpu"};

  if (!existing_keys) {
    return kExtraKeys;
  }

  Array<ffi::String> keys = existing_keys.value();
...
TargetJSON ParseTarget(TargetJSON target) {
  ...
  target.Set("keys", MergeKeys(Downcast<Optional<Array<ffi::String>>>(target.Get("keys"))));
