...
class TargetInternal {
 public:
  ...
  static Map<ffi::String, ffi::Any> Export(Target target) { return target->Export(); }
  static const TargetKindNode::ValueTypeInfo& FindTypeInfo(const TargetKind& kind,
                                                           const std::string& key);
  static Optional<ffi::String> StringifyAttrsToRaw(const Map<ffi::String, ffi::Any>& attrs);
  ...
  static ObjectPtr<Object> FromConfig(Map<ffi::String, ffi::Any> config);
 ...
 private:
  static std::unordered_map<ffi::String, ffi::Any> QueryDevice(int device_id, const TargetNode* target);
...
Any TargetInternal::ParseType(const Any& obj, const TargetKindNode::ValueTypeInfo& info) {
  if (info.type_index == ffi::TypeIndex::kTVMFFIInt) {
    ...
    if (auto opt = obj.as<Target>()) {
      ...
      Map<ffi::String, ffi::Any> config = ffi::GetRef<Map<ffi::String, ffi::Any>>(ptr);
...
Optional<ffi::String> TargetInternal::StringifyAttrsToRaw(const Map<ffi::String, ffi::Any>& attrs) {
  std::ostringstream os;
  ...
  for (const auto& key : keys) {
    ...
    if (const auto* array = obj.as<ffi::ArrayObj>()) {
      value = ffi::String(ffi::StringifyArray(*array));
    } else {
...
const std::string& TargetNode::str() const {
  if (str_repr_.empty()) {
    ...
    if (Optional<ffi::String> attrs_str = TargetInternal::StringifyAttrsToRaw(attrs)) {
      os << ' ' << attrs_str.value();
    }
...
Target::Target(const Map<ffi::String, ffi::Any>& config) {
  ObjectPtr<Object> target;
  try {
...
Target::Target(TargetKind kind, Optional<ObjectRef> host, ffi::String tag, Array<ffi::String> keys,
...
std::unordered_set<std::string> TargetNode::GetLibs() const {
  Optional<Array<ffi::String>> libs = this->GetAttr<Array<ffi::String>>("libs");
  if (!libs.defined()) {
    return {};
  }
...
Map<ffi::String, ffi::Any> TargetNode::Export() const {
...
void TargetInternal::ConstructorDispatcher(ffi::PackedArgs args, ffi::Any* rv) {
  if (args.size() == 1) {
    ...
    } else if (auto opt_map = arg.try_cast<Map<ffi::String, ffi::Any>>()) {
...
ObjectPtr<Object> TargetInternal::FromConfigString(const ffi::String& config_str) {
  ...
  auto config = (*loader)(config_str).cast<Optional<Map<ffi::String, ffi::Any>>>();
  if (!config.defined()) {
...
ObjectPtr<Object> TargetInternal::FromConfig(Map<ffi::String, ffi::Any> config) {
    ...
    if (auto kind = config[kKind].try_cast<ffi::String>()) {
      ...
      if (target->kind->target_parser != nullptr) {
        ...
        if (config.count(kFeatures)) {
          target->features = Downcast<Map<ffi::String, ffi::Any>>(config[kFeatures]);
  ...
  if (target->kind->preprocessor != nullptr) {
    target->attrs =
        target->kind->preprocessor(Map<ffi::String, ffi::Any>(attrs)).cast<Map<ffi::String, ffi::Any>>();
  } else {
    target->attrs = attrs;
  }
...
TVM_FFI_STATIC_INIT_BLOCK({
  ...
  refl::GlobalDef()
      ...
      .def("target.TargetGetFeature", [](const Target& target, const ffi::String& feature_key) -> Any {
