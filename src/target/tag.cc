...
Optional<Target> TargetTag::Get(const ffi::String& target_tag_name) {
...
Map<ffi::String, Target> TargetTag::ListTags() {
  Map<ffi::String, Target> result;
...
Target TargetTag::AddTag(ffi::String name, Map<ffi::String, ffi::Any> config, bool override) {
...
TVM_REGISTER_TARGET_TAG("raspberry-pi/4b-aarch64")
    .set_config({{"kind", ffi::String("llvm")},
                 ...
                 {"mattr", Array<ffi::String>{"+neon"}},
                 {"num-cores", 4},
                 {"host", Map<ffi::String, ffi::Any>{{"kind", ffi::String("llvm")},
                                                {"mtriple", ffi::String("aarch64-linux-gnu")},
                                                {"mcpu", ffi::String("cortex-a72")},
                                                {"mattr", Array<ffi::String>{"+neon"}},
                                                {"num-cores", 4}}}});
...
#define TVM_REGISTER_CUDA_TAG(Name, Arch, SharedMem, RegPerBlock) \
  TVM_REGISTER_TARGET_TAG(Name).set_config({                      \
      ...
      {"keys", Array<ffi::String>{"cuda", "gpu"}},                     \
