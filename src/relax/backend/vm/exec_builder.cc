...
TVM_FFI_STATIC_INIT_BLOCK({
  ...
  refl::GlobalDef()
    ...
    .def_packed("relax.ExecBuilderConvertConstant", [](ffi::PackedArgs args, ffi::Any* ret) {
      ExecBuilder builder = args[0].cast<ExecBuilder>();
      ffi::Any rt;
      rt = args[1];
      *ret = builder->ConvertConstant(rt).data();
    })
    .def("relax.ExecBuilderEmitFunction", [](ExecBuilder builder, String func, int64_t num_inputs,
                       Optional<Array<String>> param_names) {
      builder->EmitFunction(func, num_inputs, param_names);
    })
    .def_method("relax.ExecBuilderEndFunction", &ExecBuilderNode::EndFunction)
    .def("relax.ExecBuilderDeclareFunction", [](ExecBuilder builder, String name, int32_t kind) {
      builder->DeclareFunction(name, static_cast<VMFuncInfo::FuncKind>(kind));
    })
    .def("relax.ExecBuilderEmitCall", [](ExecBuilder builder, String name, Array<IntImm> args, int64_t dst) {
    ...
    .def("relax.ExecBuilderEmitRet", [](ExecBuilder builder, int64_t data) {
      builder->EmitRet(Instruction::Arg::FromData(data));
    })
    .def_method("relax.ExecBuilderEmitGoto", &ExecBuilderNode::EmitGoto)
    .def("relax.ExecBuilderEmitIf", [](ExecBuilder builder, int64_t data, vm::Index false_offset) {
      builder->EmitIf(Instruction::Arg::FromData(data), false_offset);
    })
    .def("relax.ExecBuilderR", [](ExecBuilder builder, int64_t value) {
      return Instruction::Arg::Register(value).data();
    })
    .def("relax.ExecBuilderImm", [](ExecBuilder builder, int64_t value) {
      return Instruction::Arg::Immediate(value).data();
    })
    .def("relax.ExecBuilderC", [](ExecBuilder builder, int64_t value) {
      return Instruction::Arg::ConstIdx(value).data();
    })
    .def("relax.ExecBuilderF", [](ExecBuilder builder, String value) {
