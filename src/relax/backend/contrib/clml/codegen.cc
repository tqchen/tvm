 ...
 public:
  ...
  void SetGenericAttributes(const CallNode* call_node) {
    if (backend::IsOp(call_node, "relax.nn.relu")) {
      node_->SetAttr("activation_type", ffi::Array<ffi::String>{"relu"});
    }
 ...
 public:
  ...
  std::shared_ptr<JSONGraphNode> CreateCompositeConvJSONNode(const CallNode* cn) {
    ...
    if (nodes.pad) {
      ...
      json_node->SetAttr("padding",
                         ffi::Array<ffi::String>{std::to_string(p[4].as<IntImmNode>()->value),
    ...
    if (nodes.activation) {
      json_node->SetAttr("activation_type", ffi::Array<ffi::String>{nodes.act_type});
    }
    return json_node;
  }
  ...
  static void SaveGlobalAttributes(std::shared_ptr<JSONGraphNode> node) {
    ...
    node->SetAttr("clml_version",
                  ffi::Array<ffi::String>{std::to_string(cfg.value()->clml_version.IntValue())});
  }
