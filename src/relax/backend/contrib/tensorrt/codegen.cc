...
class TensorRTJSONSerializer : public JSONSerializer {
 public:
  ...
  static void SaveGlobalAttributes(std::shared_ptr<JSONGraphNode> node) {
    ...
    ffi::Array<ffi::String> tensorrt_version = {
    ...
    ffi::Array<ffi::String> use_implicit_batch = {std::to_string(cfg.value()->use_implicit_batch)};
    ffi::Array<ffi::String> max_workspace_size = {std::to_string(cfg.value()->max_workspace_size)};
    ffi::Array<ffi::String> use_fp16 = {std::to_string(cfg.value()->use_fp16)};
    ffi::Array<ffi::String> use_uint8 = {std::to_string(cfg.value()->use_uint8)};
    node->SetAttr("tensorrt_version", tensorrt_version);
    node->SetAttr("use_implicit_batch", use_implicit_batch);
    node->SetAttr("max_workspace_size", max_workspace_size);
    node->SetAttr("use_fp16", use_fp16);
    node->SetAttr("use_uint8", use_uint8);
  }
