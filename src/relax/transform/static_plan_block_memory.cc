...
class StorageAllocatorInit : public StorageAllocatorBaseVisitor {
 ...
 private:
  ...
  Tokens CreateToken(const CallNode* call) {
    ...
    TVM_FFI_ICHECK(DataType(ty->dtype->dtype) == call->args[1].as_or_throw<DataTypeImm>()->value);
    ...
    StorageToken token(upper_bounded_shape, DataType(ty->dtype->dtype), storage_scope->value,
                       vdevice);
  ...
  Expr VisitExpr_(const CallNode* call) final {
    ...
    if (it != alloc_tensor2token_.end()) {
      ...
      if (!IsStaticShape(shape->values)) {
        TVM_FFI_ICHECK(!ty->IsUnknownDtype());
        TVM_FFI_ICHECK_EQ(DataType(ty->dtype->dtype),
