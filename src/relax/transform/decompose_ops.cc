...
Tuple DecomposeBatchNorm(const Call& call) {
  ...
  Expr epsilon = MakeConstantScalar(attrs->epsilon, DataType(ty->dtype->dtype));
...
Expr MutateBatchNormForTraining(Call call) {
  ...
  Expr momentum = MakeConstantScalar(attrs->momentum, DataType(ty->dtype->dtype));
  Expr one_minus_mom = MakeConstantScalar(1 - attrs->momentum, DataType(ty->dtype->dtype));
