...
Type InferTypeMultinomialFromUniform(const Call& call, const BlockBuilder& ctx) {
  ...
  if (prob_ty->dtype->dtype.code != DLDataTypeCode::kDLFloat &&
      prob_ty->dtype->dtype.code != DLDataTypeCode::kDLBfloat) {
  ...
  // Only the element kind matters here; shape inference does not depend on vector lanes.
  if (uniform_sample_ty->dtype->dtype.code != DLDataTypeCode::kDLFloat &&
      uniform_sample_ty->dtype->dtype.code != DLDataTypeCode::kDLBfloat) {
    TVM_FFI_VISIT_THROW(TypeError, call)
  ...
  // Only the element kind matters here; shape inference does not depend on vector lanes.
  if (sample_indices_ty->dtype->dtype.code != DLDataTypeCode::kDLInt) {
    TVM_FFI_VISIT_THROW(TypeError, call)
