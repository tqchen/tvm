...
Expr attention(Expr query, Expr key, Expr value, Optional<Expr> bias, Optional<FloatImm> scale,
               Optional<ffi::String> causal_mask, Optional<IntImm> window_size) {
...
StructInfo InferStructInfoAttention(const Call& call, const BlockBuilder& ctx) {
  ...
  auto multiple_of = [&](PrimExpr v1, PrimExpr v2, ffi::String m1, ffi::String m2, ffi::String dim) {
  ...
  if (input_sinfo.size() == 4) {
    ...
    auto diag_equal_or_broadcast = [&](PrimExpr v1, PrimExpr v2, ffi::String m1, ffi::String m2, ffi::String dim) {
