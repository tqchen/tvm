...
class InstrumentIntrin : public StmtMutator {
 public:
  ...
  Stmt VisitStmt_(const ForNode* op) final {
    ...
    PrimExpr start_call = tvm::Call(PrimType::Handle(), builtin::start_profile_intrinsic(), {id})
                              .as_or_throw<PrimExpr>();
    PrimExpr end_call = tvm::Call(PrimType::Handle(), builtin::end_profile_intrinsic(), {id})
                            .as_or_throw<PrimExpr>();
...
PrimFunc AddProfileBuiltins(PrimFunc func, int32_t max_instr_depth, int32_t min_instr_height,
  ...
  if (!disable_func_instrumentation) {
    PrimExpr start_call = tvm::Call(PrimType::Handle(), builtin::start_profile_intrinsic(), {e})
                              .as_or_throw<PrimExpr>();
    PrimExpr end_call = tvm::Call(PrimType::Handle(), builtin::end_profile_intrinsic(), {e})
                            .as_or_throw<PrimExpr>();
    const Stmt start_profile = Evaluate(start_call);
    const Stmt end_profile = Evaluate(end_call);
