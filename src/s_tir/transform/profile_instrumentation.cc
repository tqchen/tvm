...
class InstrumentIntrin : public StmtMutator {
 public:
  ...
  Stmt VisitStmt_(const ForNode* op) final {
    ...
    PrimExpr start_call = tirx::Call(PrimType::Handle(), builtin::start_profile_intrinsic(), {id});
    PrimExpr end_call = tirx::Call(PrimType::Handle(), builtin::end_profile_intrinsic(), {id});
...
PrimFunc AddProfileBuiltins(PrimFunc func, int32_t max_instr_depth, int32_t min_instr_height,
  ...
  if (!disable_func_instrumentation) {
    PrimExpr start_call = tirx::Call(PrimType::Handle(), builtin::start_profile_intrinsic(), {e});
    PrimExpr end_call = tirx::Call(PrimType::Handle(), builtin::end_profile_intrinsic(), {e});
    const Stmt start_profile = Evaluate(start_call);
    const Stmt end_profile = Evaluate(end_call);
