...
IRModule ModuleReplaceGlobalVars(
    IRModule mod, Map<Variant<ffi::String, GlobalVar>, Variant<ffi::String, GlobalVar>> replacements) {
  ...
  for (const auto& [before, after] : replacements) {
    ...
    if (auto gvar = before.as<GlobalVar>()) {
      ...
      LOG(FATAL) << "Variant<ffi::String,GlobalVar> must contain either ffi::String or GlobalVar";
    }
