  ...
  void LoadGlobalOptions() {
    ...
    for (size_t i = 0; i < nodes_.size(); ++i) {
      if (nodes_[i].HasAttr("msc_global_options_num")) {
        engine_file_ = nodes_[i].GetAttr<ffi::Array<ffi::String>>("msc_global_engine")[0];
        graph_name_ = nodes_[i].GetAttr<ffi::Array<ffi::String>>("msc_global_graph_name")[0];
        if (nodes_[i].HasAttr("msc_global_tool_tag")) {
          tool_tag_ = nodes_[i].GetAttr<ffi::Array<ffi::String>>("msc_global_tool_tag")[0];
        } else {
          tool_tag_ = "";
        }
      }
