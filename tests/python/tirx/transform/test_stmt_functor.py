...
def test_for_with_break_continue():
    ...
    Python ``break`` / ``continue`` keywords lower to
    ``T.evaluate(T.break_loop())`` / ``T.evaluate(T.continue_loop())``
    (Evaluate + Call) rather than dedicated Break / Continue Stmt nodes.
    """
