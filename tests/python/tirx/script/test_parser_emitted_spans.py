...
# under the License.
"""Native emitted statements and frames preserve source locations."""
...
def test_emit_ordinary_value_once():
    with IRBuilder() as builder:
        with T.function():
            T.func_name("single")
            value = I.at_(loc(4), tirx.IntImm("int32", 2))
            T.emit(value)
...
def test_native_frame_retains_span_until_exit():
    with IRBuilder() as builder:
        ...
        assert I.at_(loc(6), frame) is frame
        with frame:
            receipt = I.with_at_group_(loc(7), lambda: T.evaluate(1))
    result = builder.get()
...
def test_while_frame_retains_span_until_exit():
    with IRBuilder() as builder:
        frame = T.While(tirx.IntImm("bool", 1))
        assert I.at_(loc(8), frame) is frame
        with frame:
            T.evaluate(0)
...
def test_at_retains_definition_after_caller_scope_returns():
    with IRBuilder() as builder:
        ...
        def helper():
            return I.with_at_group_(loc(2, "definition.py"), lambda: T.evaluate(1))

        receipt = I.with_at_group_(loc(9, "caller.py"), helper)
        assert I.at_(loc(9, "caller.py"), receipt) is receipt
        T.emit(receipt)
...
def test_nested_frame_context_has_no_repeated_caller():
    with IRBuilder() as builder:
        with builder.with_source_span(ir.Span(ir.SourceName("caller.py"), 1, 1, 1, 20)):
            frame = I.with_at_group_(loc(2, "definition.py"), lambda: native.serial(0, 2))
            with frame:
                T.evaluate(0)
...
def test_tile_insertion_composes_caller_on_existing_statement_span():
    ...
    with IRBuilder() as builder:
        statement = I.at_(loc(2, "definition.py"), tile_operator.Zero(buffer[:], buffer[:]))
        result = I.with_at_group_(loc(9, "caller.py"), lambda: tile_builder.f_insert(statement))
