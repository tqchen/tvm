...
def test_add():
    ...
    def check_c():
        mhost = tvm.build(
            tvm.IRModule.from_expr(te.create_prim_func([A, B, C])
                .with_attr("global_symbol", "test_fadd")),
            target="c"
        )
        temp = utils.tempdir()
...
def test_reinterpret():
    ...
    def check_c():
        mhost = tvm.build(
            tvm.IRModule.from_expr(te.create_prim_func([A, B])
                .with_attr("global_symbol", "test_reinterpret")),
            target="c"
        )
        temp = utils.tempdir()
        path_dso = temp.relpath("temp.so")
...
def test_ceil():
    ...
    def check_c():
        mhost = tvm.build(
            tvm.IRModule.from_expr(te.create_prim_func([A, B])
                .with_attr("global_symbol", "test_ceil")),
            target="c"
        )
        temp = utils.tempdir()
        path_dso = temp.relpath("temp.so")
...
def test_floor():
    ...
    def check_c():
        mhost = tvm.build(
            tvm.IRModule.from_expr(te.create_prim_func([A, B])
                .with_attr("global_symbol", "test_floor")),
            target="c"
        )
        temp = utils.tempdir()
        path_dso = temp.relpath("temp.so")
...
def test_round():
    ...
    def check_c():
        mhost = tvm.build(
            tvm.IRModule.from_expr(te.create_prim_func([A, B])
                .with_attr("global_symbol", "test_round")),
            target="c"
        )
        temp = utils.tempdir()
        path_dso = temp.relpath("temp.so")
