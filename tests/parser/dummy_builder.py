...
Values carry only an operation and operands. Frames record entry, parameters,
outputs and identity; they deliberately implement no TVM type system or parser.
...
@dataclass(eq=False)
class Value:
    """An opaque expression with observable operands and source locations."""
...
class Frame:
    ...
    def __init__(self, language, kind, *, decl=False, values=(), span=None, local=False):
    ...
    def __exit__(self, error_type, error, traceback):
        ...
        if self.kind == "function":
            ...
            self.language.result = self.language.functions
        return False
...
class Language:
    ...
    def func_ret_type(self, annotation):
        self.frame().function.ret_type = annotation
