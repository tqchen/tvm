...
Each dialect registers its namespace through ``register_namespace`` and supplies
...
``tvm.script.ir_builder.ir.parser_protocol``; concrete dialects own registration
...
def args_policy(
    ...
    *,
    ...
    dtype : object
        Opaque dtype passed to the dialect's symbol resolver.
    as_type : bool
    ...
    .. code:: python
        ...
        @args_policy("X.tensor", {"shape": "expr_str"})
        def tensor(shape):
            ...
        # Source: X.tensor(("n",))
...
def result_span(namespace_path: str) -> Callable[[_Callable], _Callable]:
    ...
    assignment RHS locations separately to the dialect's ``bind_``.
