 ...
 private:
  ...
  void WriteValue(const json::Value& value) {
    switch (value.type_index()) {
     ...
     case TypeIndex::kTVMFFIBool: {
       bool bool_value = value.cast<bool>();
       if (bool_value) {
         WriteLiteral("true", 4);
       } else {
     ...
     case TypeIndex::kTVMFFIInt: {
       WriteInt(value.cast<int64_t>());
       break;
     }
     case TypeIndex::kTVMFFIFloat: {
       WriteFloat(value.cast<double>());
       break;
     }
     case TypeIndex::kTVMFFISmallStr:
     case TypeIndex::kTVMFFIStr: {
       WriteString(value.cast<String>());
       break;
     }
     case TypeIndex::kTVMFFIArray: {
       WriteArray(value.cast<json::Array>());
       break;
     }
     case TypeIndex::kTVMFFIMap: {
       WriteObject(value.cast<json::Object>());
       break;
     }
     default: {
  ...
  void WriteFloat(double value) {
    ...
    if (std::isnan(value)) {
      ...
      if (std::fabs(int_part) <  (1ULL << 53) && std::modf(value, &int_part) == 0) {
...
#undef HANDLE_ESCAPE_CHAR
        default: {
          ...
          if (u8_val < 0x20 || u8_val == 0x7f) {
            char buffer[8];
            int size = TVM_FFI_SNPRINTF(buffer, sizeof(buffer), "\\u%04x", static_cast<int32_t>(data[i]) & 0xff);
            WriteLiteral(buffer, size);
          } else {
            *out_iter_++ = data[i];
          }
