...
class JSONParserContext {
 public:
   ...
   */
  TVM_FFI_INLINE void SkipNextAssumeNoSpace() { ++cur_; }
 ...
 private:
  ...
  bool ParseNumber(json::Value* out) {
    ...
    if (maybe_int) {
      ...
      char* end_ptr;
      errno = 0;
      intmax_t int_val = strtoimax(temp_buffer_.data(), &end_ptr, 10);
      if (errno == 0 && int_val >= std::numeric_limits<int64_t>::min() &&
          int_val <= std::numeric_limits<int64_t>::max() &&
          end_ptr == temp_buffer_.data() + temp_buffer_.size()) {
    ...
    }
    {
      ...
      char* end_ptr;
      errno = 0;
      double double_val = strtod(temp_buffer_.data(), &end_ptr);
      if (errno == 0 && end_ptr == temp_buffer_.data() + temp_buffer_.size()) {
        *out = double_val;
        return true;
      } else {
        ctx_.SetCurrentPosForBetterErrorMsg(start_pos);
