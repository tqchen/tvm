...
TEST(JSONParser, Array) {
  EXPECT_TRUE(refl::StructuralEqual()(json::Parse("[]"), json::Array{}));

  EXPECT_TRUE(refl::StructuralEqual()(json::Parse("[1, 2,\n\t\"a\"]"), json::Array{1, 2, "a"}));
}
...
TEST(JSONParser, Object) {
  EXPECT_TRUE(refl::StructuralEqual()(json::Parse("{}"), json::Object{}));

  EXPECT_TRUE(refl::StructuralEqual()(json::Parse("{\"a\":  1, \n\"b\": \t\"c\"}   "),
                                      json::Object{{"a", 1}, {"b", "c"}}));
}
...
TEST(JSONParser, ObjectOrderPreserving) {
  ...
  EXPECT_TRUE(refl::StructuralEqual()(keys, json::Array{"c", "a", "b"}));
}
...
TEST(JSONParser, NestedObject) {
  EXPECT_TRUE(refl::StructuralEqual()(
      json::Parse("{\"a\": \t{\"b\": 1}, \n\"c\": [1, 2, 3]}"),
...
TEST(JSONParser, WhitespaceHandling) {
  ...
  EXPECT_TRUE(
      refl::StructuralEqual()(json::Parse("  [  1  ,  2  ,  3  ]  "), json::Array{1, 2, 3}));

  EXPECT_TRUE(refl::StructuralEqual()(json::Parse("  {  \"a\"  :  1  ,  \"b\"  :  2  }  "),
                                      json::Object{{"a", 1}, {"b", 2}}));
}
...
TEST(JSONParser, DuplicateKeys) {
  ...
  EXPECT_EQ(obj.at("a").cast<int64_t>(), 3);
}
...
TEST(JSONParser, MixedDataTypes) {
  ...
  EXPECT_TRUE(refl::StructuralEqual()(result, expected));
}
