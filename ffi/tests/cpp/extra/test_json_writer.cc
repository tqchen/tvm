...
TEST(JSONWriter, Integer) {
  ...
  EXPECT_EQ(json::Stringify(json::Value(std::numeric_limits<int64_t>::max())), "9223372036854775807");
  // large negative integer
  EXPECT_EQ(json::Stringify(json::Value(std::numeric_limits<int64_t>::min())), "-9223372036854775808");
}
...
TEST(JSONWriter, String) {
  ...
  EXPECT_EQ(json::Stringify(json::Value(String(std::string("\x01", 1) + "control"))), "\"\\u0001control\"");
}
...
TEST(JSONWriter, Object) {
  ...
  json::Object multi_object{
    {"name", "Alice"},
    {"age", 30},
    {"active", true},
    {"score", 95.5}
  };
  EXPECT_EQ(json::Stringify(multi_object), "{\"name\":\"Alice\",\"age\":30,\"active\":true,\"score\":95.5}");
}
...
TEST(JSONWriter, InsertionOrderPreservation) {
  ...
  EXPECT_EQ(json::Stringify(ordered_object),
...
TEST(JSONWriter, NestedStructures) {
  ...
  json::Object nested_obj{{String("nested"), json::Array{json::Object{{String("deep"), String("value")}}}}};
