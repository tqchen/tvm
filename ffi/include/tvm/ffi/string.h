...
class String {
 public:
  ...
  String(std::string&& other) {  // NOLINT(*)
    ...
    data_.v_obj = details::ObjectUnsafe::MoveObjectPtrToTVMFFIObjectPtr(
        make_object<details::BytesObjStdImpl<StringObj>>(std::move(other)));
 ...
 private:
  ...
  char* InitSpaceForSize(size_t size) {
    // need to reserve one byte for \0
    ...
    if (size < kMaxSmallStrLen) {
      ...
      // allocate from heap
      ObjectPtr<StringObj> ptr = make_inplace_array_object<StringObj, char>(size + 1);
      char* dest_data = reinterpret_cast<char*>(ptr.get()) + sizeof(StringObj);
      ptr->data = dest_data;
      ptr->size = size;
