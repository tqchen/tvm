 ...
 *
 * Provides:
 *   - tvm::support::Stream        (abstract binary stream)
 *   - tvm::support::Serializer<T> (primary template; specialize for each type)
 *
 * Stream::Write<T>() and Stream::Read<T>() delegate to Serializer<T>.
 * Specializations live in tvm/support/serializer.h.
 */
 ...
 * The template Write<T>/Read<T> methods delegate to Serializer<T>
